...
public final class MentionNotificationController {
	...
	// private final MentionToastManager toastManager = new MentionToastManager(); // TODO
	...
	public void receive(ChatCanvasMessage message) {
		...
		if (!deduplicator.accept(message.messageId(), message.receivedAt())) {
			ChatCanvasForge.LOGGER.debug("Mention notification suppressed as duplicate: id={}",
					message.messageId());
			return;
		}
