...
import io.github.ikunkk02.chatcanvas.chat.identity.ChatMessageMetadata;
...
public final class DefaultMessageClassifier implements MessageClassifier {
	...
	public ClassifiedMessage classify(Component message, MessageContext context) {
		...
		var plugin = java.util.Optional.<io.github.ikunkk02.chatcanvas.chat.identity.ChatMessageMetadata>empty();
	...
	private static ChatCanvasMessageSource vanillaSource(Component message) {
		if (!(message.getContents() instanceof net.minecraft.network.chat.contents.TranslatableContents translated)) return null;
