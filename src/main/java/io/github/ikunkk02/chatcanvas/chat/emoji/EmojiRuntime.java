...
public final class EmojiRuntime {
	private static final Path DATA_PATH = java.nio.file.Path.of("config").resolve("chat_canvas").resolve("emoji.json");
	...
	public static void flush() {
		if (recent != null && !recent.flush()) {
			ChatCanvasForge.LOGGER.error("Failed to save Emoji recent-use data at {}", DATA_PATH);
		}
	}
}
