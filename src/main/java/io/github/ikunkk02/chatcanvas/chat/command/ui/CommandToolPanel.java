...
public final class CommandToolPanel {
	...
	public void render(
			ChatScreen screen, EditBox commandField, GuiGraphics context,
		...
		fillButton(context, px + panelWidth - 32, y + 43, 24, 18,
				mouseX, mouseY, false);
		context.drawCenteredString(renderer,
		...
		if (statusKey != null) {
			context.drawString(renderer, Component.translatable(statusKey),
	...
	private void renderTabs(GuiGraphics context, Font renderer, int px,
	...
	private void renderToolbar(GuiGraphics context, Font renderer, int px,
			int mouseX, int mouseY) {
		...
		context.drawCenteredString(renderer, Component.literal(fittedPrimary),
	...
	private void renderRow(
			GuiGraphics context, Font renderer, Row row, int index,
		...
		if (hover && renderer.width(row.command()) > panelWidth - 34) {
			context.renderTooltip(renderer, Component.literal(row.command()), mouseX, mouseY);
		} else if (hover && row.dangerous()) {
			context.renderTooltip(renderer,
	...
	private void renderDialog(
			GuiGraphics context, Font renderer,
			int mouseX, int mouseY, float delta) {
		int px = visualX;
		int dx = px + 8;
		int dy = y + 22;
		...
		context.drawString(renderer, Component.translatable(dialog.titleKey),
		...
		fillButton(context, confirmX, y + panelHeight - 32, buttonWidth, 18,
				mouseX, mouseY, true);
		context.drawCenteredString(renderer, Component.translatable("gui.cancel"),
				cancelX + buttonWidth / 2, y + panelHeight - 27,
				ModernUiTheme.TEXT_PRIMARY);
		context.drawCenteredString(renderer, Component.translatable("gui.ok"),
	...
	private static void fillButton(
			GuiGraphics context, int x, int y,
