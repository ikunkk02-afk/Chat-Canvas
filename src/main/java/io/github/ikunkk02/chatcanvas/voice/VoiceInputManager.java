...
public final class VoiceInputManager {
	...
	private void startSession(long token) {
		try {
			...
			try {
				...
				ChatCanvasForge.LOGGER.warn("Voice session resource unavailable: {}", ise.getMessage());
				cleanupAfterAbort();
			} catch (Throwable throwable) {
				if (isMicrophoneAccessIssue(throwable)) {
					fail("chat_canvas.voice.error.microphone", throwable);
				} else {
					ChatCanvasForge.LOGGER.error("Voice session resource release failed", throwable);
					cleanupAfterAbort();
				}
			}
		});
	...
	private void fail(String key, Throwable throwable) {
		ChatCanvasForge.LOGGER.error("Voice input failure", throwable);
		onClient(() -> {
			synchronized (this) {
				VoiceInputSession active = session;
				session = null;
