...
public final class VoiceInputOverlay {
	...
	private void renderButton(GuiGraphics context, int mouseX, int mouseY) {
		...
		if (manager.isListening()) {
			context.renderOutline(buttonX, buttonY, BUTTON_WIDTH, BUTTON_HEIGHT, ModernUiTheme.DANGER);
		} else if (state == VoiceInputState.MODEL_MISSING || state == VoiceInputState.ERROR) {
			context.renderOutline(buttonX, buttonY, BUTTON_WIDTH, BUTTON_HEIGHT, ModernUiTheme.WARNING);
		}
		int cx = buttonX + 9;
	...
	private void renderPrompt(GuiGraphics context, int mouseX, int mouseY) {
		PromptLayout layout = promptLayout();
	...
	private void drawModelCard(GuiGraphics context, VoiceModelDescriptor model,
	...
	private static void button(GuiGraphics context, int mouseX, int mouseY, int x, int y,
	...
	private static void draw(GuiGraphics context, String key, int x, int y, int color) {
		context.drawString(Minecraft.getInstance().font, Component.translatable(key), x, y, color);
	}

	private static void drawFitted(GuiGraphics context, String value, int x, int y, int width, int color) {
		String fitted = ModernUiTheme.fitText(Minecraft.getInstance().font,
				Component.literal(value), Math.max(1, width));
		context.drawString(Minecraft.getInstance().font, Component.literal(fitted), x, y, color);
	}
