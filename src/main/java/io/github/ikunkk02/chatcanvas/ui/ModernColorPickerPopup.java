...
public final class ModernColorPickerPopup extends BaseComponent {
	...
	public void draw(OwoUIDrawContext context, int mouseX, int mouseY, float partialTicks, float delta) {
		...
		context.drawString(renderer, Component.translatable("chat_canvas.color_picker.title"),
	...
	private void drawHexInput(OwoUIDrawContext context, Font renderer) {
		context.drawString(renderer, Component.translatable("chat_canvas.color_picker.hex"),
		...
		String visible = renderer.plainSubstrByWidth(text, metrics.svWidth() - 12, true);
		context.drawString(renderer, visible, x() + metrics.svX() + 6,
				y() + metrics.hexY() + (metrics.hexHeight() - renderer.lineHeight) / 2,
		...
		if (hexFocused && (System.currentTimeMillis() / 350L) % 2L == 0L) {
			int cursorX = Math.min(x() + metrics.svX() + metrics.svWidth() - 5,
					x() + metrics.svX() + 6 + renderer.width(visible));
		...
		if (!state.hexValid() && !metrics.compact()) {
			context.drawString(renderer, Component.translatable("chat_canvas.color_picker.invalid"),
					x() + metrics.svX(), y() + metrics.hexY() + metrics.hexHeight() + 3,
					ModernUiTheme.DANGER, false);
		}
	}

	private void drawRecentColors(OwoUIDrawContext context, Font renderer) {
		context.drawString(renderer, Component.translatable("chat_canvas.color_picker.recent"),
	...
	private void drawActions(OwoUIDrawContext context, Font renderer,
	...
	private static void drawAction(OwoUIDrawContext context, Font renderer,
		...
		int textX = x + Math.max(3, (width - renderer.width(fitted)) / 2);
		int textY = y + (height - renderer.lineHeight) / 2;
		context.drawString(renderer, fitted, textX, textY, color, false);
	}
