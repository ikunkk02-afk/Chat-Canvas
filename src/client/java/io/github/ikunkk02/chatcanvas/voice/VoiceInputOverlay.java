...
public final class VoiceInputOverlay {
	...
	private void renderButton(DrawContext context, int mouseX, int mouseY) {
		...
		if (manager.isListening()) {
			ChatBackgroundDraw.drawRectBorder(context, buttonX, buttonY,
					buttonX + BUTTON_WIDTH, buttonY + BUTTON_HEIGHT, ModernUiTheme.DANGER);
		} else if (state == VoiceInputState.MODEL_MISSING || state == VoiceInputState.ERROR) {
			ChatBackgroundDraw.drawRectBorder(context, buttonX, buttonY,
					buttonX + BUTTON_WIDTH, buttonY + BUTTON_HEIGHT, ModernUiTheme.WARNING);
		}
		int cx = buttonX + 9;
