...
public final class VoiceInputOverlay {
	...
	private void renderButton(DrawContext context, int mouseX, int mouseY) {
		...
		if (manager.isListening()) {
			ChatBackgroundDraw.drawBorder(context, buttonX, buttonY, BUTTON_WIDTH, BUTTON_HEIGHT, ModernUiTheme.DANGER);
		} else if (state == VoiceInputState.MODEL_MISSING || state == VoiceInputState.ERROR) {
			ChatBackgroundDraw.drawBorder(context, buttonX, buttonY, BUTTON_WIDTH, BUTTON_HEIGHT, ModernUiTheme.WARNING);
		}
		int cx = buttonX + 9;
