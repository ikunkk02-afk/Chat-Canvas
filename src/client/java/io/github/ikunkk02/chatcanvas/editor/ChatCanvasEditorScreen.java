...
public final class ChatCanvasEditorScreen extends BaseOwoScreen<FlowLayout> {
	...
	public boolean keyPressed(KeyInput input) {
		...
		if (net.minecraft.client.MinecraftClient.getInstance().isCtrlPressed() && keyCode == GLFW.GLFW_KEY_Z) {
			undo();
			return true;
		}
		if (net.minecraft.client.MinecraftClient.getInstance().isCtrlPressed() && keyCode == GLFW.GLFW_KEY_Y) {
			redo();
			return true;
		}
		...
		if (scrubber != null) {
			if (button == GLFW.GLFW_MOUSE_BUTTON_LEFT) {
				return pointerCapture.begin(scrubber, mouseX, mouseY, button,
						net.minecraft.client.MinecraftClient.getInstance().isShiftPressed(), net.minecraft.client.MinecraftClient.getInstance().isCtrlPressed());
			}
