...
public final class EmojiPickerPanel {
	...
	public boolean mouseClicked(double mouseX, double mouseY, int button) {
		...
		if (adapter != null && adapter.mouseClicked(
				new Click(mouseX - x, mouseY - y, new MouseInput(button, 0)), false)) {
	...
	public boolean keyPressed(int keyCode, int scanCode, int modifiers) {
		...
		if (focus == FocusTarget.SEARCH) {
			return adapter != null
					&& adapter.keyPressed(new KeyInput(keyCode, scanCode, modifiers));
		}
	...
	public boolean charTyped(int codepoint, int modifiers) {
		return open && focus == FocusTarget.SEARCH && adapter != null
				&& adapter.charTyped(new CharInput(codepoint, modifiers));
	}
