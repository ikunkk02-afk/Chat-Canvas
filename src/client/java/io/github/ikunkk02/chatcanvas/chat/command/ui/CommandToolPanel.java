...
public final class CommandToolPanel {
	...
	public static boolean dispatchCharTyped(
			ChatScreen screen, int codepoint, int modifiers) {
		CommandToolPanel panel = ACTIVE.get(screen);
		return panel != null && panel.charTyped(codepoint, modifiers);
	}
	...
	public boolean mouseClicked(
		...
		if (searchField.mouseClicked(
				new Click(mouseX, mouseY, new MouseInput(button, 0)), false)) {
		...
		if (button == GLFW.GLFW_MOUSE_BUTTON_RIGHT) {
			...
			insert(commandField, suggestor, row.command(), net.minecraft.client.MinecraftClient.getInstance().isShiftPressed());
		}
		return true;
	}
	...
	public boolean keyPressed(
		...
		if (!open) {
			if (keyCode == GLFW.GLFW_KEY_F && net.minecraft.client.MinecraftClient.getInstance().isCtrlPressed()) {
				open = true;
				setSearchFocused(true);
				return true;
			}
		...
		if (dialog != Dialog.NONE) {
			...
			if (dialog == Dialog.EDIT_FAVORITE
				...
				return focusedDialogField().keyPressed(
						new KeyInput(keyCode, scanCode, modifiers));
			}
			return true;
		}
		if (keyCode == GLFW.GLFW_KEY_ESCAPE) {
		...
		if (searchField.isFocused()) {
			...
			return searchField.keyPressed(new KeyInput(keyCode, scanCode, modifiers));
		}
		...
		if (keyCode == GLFW.GLFW_KEY_ENTER
			...
			if (!rows.isEmpty()) {
				insert(commandField, suggestor,
						...
						net.minecraft.client.MinecraftClient.getInstance().isShiftPressed());
			}
			return true;
		}
	...
	public boolean charTyped(int codepoint, int modifiers) {
		if (dialog == Dialog.ADD_FAVORITE
			...
			return focusedDialogField().charTyped(new CharInput(codepoint, modifiers));
		}
		return open && searchField.isFocused()
				&& searchField.charTyped(new CharInput(codepoint, modifiers));
	}
	...
	private boolean dialogClick(double mouseX, double mouseY, int button) {
		...
		if (dialog == Dialog.ADD_FAVORITE
				|| dialog == Dialog.EDIT_FAVORITE) {
			if (nameField.mouseClicked(
					new Click(mouseX, mouseY, new MouseInput(button, 0)), false)) {
				focusDialogField(nameField);
				return true;
			}
			if (favoriteCommandField.mouseClicked(
					new Click(mouseX, mouseY, new MouseInput(button, 0)), false)) {
