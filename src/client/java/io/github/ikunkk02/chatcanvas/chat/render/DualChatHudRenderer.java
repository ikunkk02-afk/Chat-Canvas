...
public final class DualChatHudRenderer {
	...
	private void renderChannel(GuiGraphicsExtractor context, Minecraft client,
		...
		boolean open = client.gui.screen() instanceof ChatScreen;
	...
	private void drawMessage(GuiGraphicsExtractor context, Minecraft client,
		...
		for (int lineIndex = 0; lineIndex < layout.lines().size(); lineIndex++) {
			...
			context.pose().translate(textX, y);
			context.pose().scale((float) text.fontScale(), (float) text.fontScale());
			...
			if (lineIndex == 0 && headWidth > 0) {
				...
				context.pose().pushMatrix();
				context.pose().translate(headX, y);
				context.pose().scale((float) text.fontScale(), (float) text.fontScale());
	...
	private static int messageBottom(Minecraft client, ChatCanvasChannel channel,
		...
		if (!(client.gui.screen() instanceof ChatCanvasInputScreenBridge bridge)) {
			return box.bottom();
		}
	...
	private record HitLine(ChatCanvasChannel channel, ChatCanvasMessage message,
						   FormattedCharSequence text, ChatLineHitbox bounds, double spacing) {}
}
