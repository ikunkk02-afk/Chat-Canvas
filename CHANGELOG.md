...
## [1.2.0-mc1.21.7] - 2026-07-29

### Ported
- Port to Minecraft 1.21.7 (Fabric)
- Updated Fabric API to 0.129.0+1.21.7
- Updated owo-lib to 0.12.21+1.21.6
- Updated Yarn mappings to 1.21.7+build.8

### Adapted
- DrawContext.fill — 5-param overload preserved (delegates to RenderPipelines.GUI)
- DrawContext.drawText / drawTextWithShadow — return type changed to void
- DrawContext.drawTextWrapped → drawWrappedTextWithShadow (renamed)
- RenderLayer → RenderPipelines (import migration in TextFieldWidgetMixin)
- ChatHud.render — lambda refactor removed 4 @WrapOperation annotations (custom background/text rendering in vanilla fallback path)

### Removed (due to ChatHud lambda refactor)
- ChatHudMixin message background customization (fill ordinal 0/1 wrap)
- ChatHudMixin chat line text customization (drawTextWithShadow wraps)
- Note: DualChatHudRenderer still provides full custom rendering when Chat Canvas is active
